...
class EcommerceProductVariationTaskDeleteVariations extends BuildTask{
	...
	function run($request){
		...
		if($live) {
			if($this->verbose) {DB::alteration_message("this is a live task", "deleted");}
		}
		else {
			if($this->verbose) {DB::alteration_message("this is a test only. If you add a live=1 get variable then you can make it for real ;-)", "created");}
		}
		if($productID == -1) {
		...
		if($products && $products->count()) {
			foreach($products as $product) {
				...
				if($products->count()) {
					if($this->verbose) {DB::alteration_message("Deleting variations for ".$product->Title, "deleted");}
					$variations = ProductVariation::get()->filter(array("ProductID" => $productID))->limit(100);
					if($variations->count()) {
						if($this->verbose) {DB::alteration_message("PRE DELETE COUNT: ".$variations->count());}
						...
						if($live) {
							if($variations->count()) {
								if($this->verbose) {DB::alteration_message("POST DELETE COUNT: ".$variations->count());}
							}
							else {
								if($this->verbose) {DB::alteration_message("All variations have been deleted: ", "created");}
							}
						}
						else {
							if($this->verbose) {DB::alteration_message("This was a test only", "created");}
						}
					}
					else {
						if($this->verbose) {DB::alteration_message("There are no variations to delete", "created");}
					}
					if($this->verbose) {DB::alteration_message("Starting cleanup", "created");}
					if($live) {
						$sql = "
									DELETE
									FROM \"Product_VariationAttributes\"
									WHERE \"ProductID\" = ".$productID;
						if($this->verbose) {DB::alteration_message("<pre>RUNNING<br />".$sql."</pre>");}
						DB::query($sql);
						$sql = "
						...
						if($this->verbose) {DB::alteration_message("<pre>RUNNING<br />".$sql."</pre>");}
						$result = DB::query($sql);
						if($this->verbose) {DB::alteration_message("Would have deleted ".$result->value()." rows");}
						$sql = "
			...
			if($this->verbose) {DB::alteration_message("Product does not exist. You can set the product by adding it productid=XXX as a GET variable.  You can also add <i>all</i> to delete ALL product Variations.", "deleted");}
		}
